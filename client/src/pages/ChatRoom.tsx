...
import { Send, ArrowLeft, Flag, X, LogOut, Trash2, Code2, Shield, Crown, Wifi, WifiOff } from "lucide-react";
...
function UsernameBadge({ username, rank }: { username: string; rank?: string }) {
  return (
    <Link to={`/profile/${encodeURIComponent(username)}`} className="inline-flex items-center gap-1.5 group/user">
...
export default function ChatRoom() {
  ...
  const checkBanStatus = async () => {
    if (!username) return;
    try {
      const res = await fetch(`/api/user/banned/${encodeURIComponent(username)}`);
      if (res.ok) { const data = await res.json(); if (data.isBanned) setIsBanned(true); }
    } catch {}
  };
  ...
  const handleDeleteMessage = async (messageId: string) => {
    ...
    // Message removal will arrive via WebSocket broadcast
  };
  ...
  const handleReportMessage = async (messageId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessages(messages.map((msg) => msg.id === messageId ? { ...msg, reported: true, reportCount: (msg.reportCount || 0) + 1 } : msg));
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="bg-red-900/20 border border-red-500/50 rounded-lg p-8">
          ...
          <p className="text-gray-300 mb-6">Your account has been banned from the chat. If you believe this is a mistake, please contact an administrator.</p>
      ...
      <header className="bg-gray-800 border-b border-gray-700 p-4">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <span title={wsConnected ? "Real-time connected" : "Connecting..."} className={wsConnected ? "text-green-400" : "text-gray-500"}>
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/" className="flex items-center gap-2 px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            ...
            messages.map((msg) => (
              <div
                key={msg.id}
                className={`rounded-lg p-3 ${msg.flagged ? "bg-red-500/20 border border-red-500/30" : msg.reported ? "bg-orange-500/20 border border-orange-500/30" : "bg-gray-800 border border-gray-700"}`}
              >
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <UsernameBadge username={msg.username} rank={msg.rank} />
                    <p className={`mt-1 ${msg.flagged ? "text-red-300 line-through" : "text-gray-300"}`}>{msg.text}</p>
                  </div>
                  <div className="flex items-center gap-2 ml-2">
                    {canModerate && (
                      <button onClick={() => handleDeleteMessage(msg.id)} className="p-1 text-gray-500 hover:text-red-400 transition-colors" title="Delete message">
                        <Trash2 size={15} />
                      </button>
                    )}
                    <button onClick={() => setReportingMessageId(msg.id)} className="p-1 text-gray-400 hover:text-orange-400 transition-colors" title="Report message">
                      <Flag size={16} />
                    </button>
                    <span className="text-gray-500 text-xs whitespace-nowrap">{new Date(msg.timestamp).toLocaleTimeString()}</span>
                  </div>
                </div>

                {msg.flagged && <p className="text-red-400 text-xs mt-2">⚠️ Message flagged for inappropriate content</p>}
                {msg.reported && <p className="text-orange-400 text-xs mt-2">🚩 Reported by community ({msg.reportCount} report{msg.reportCount !== 1 ? "s" : ""})</p>}
                ...
                {reportingMessageId === msg.id && (
                  <div className="mt-3 p-3 bg-gray-700 rounded border border-gray-600">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <button onClick={() => setReportingMessageId(null)} className="text-gray-400 hover:text-white"><X size={16} /></button>
                    </div>
                    <select
                      value={reportReason}
        ...
        <form onSubmit={handleSendMessage} className="border-t border-gray-700 p-4 bg-gray-800">
          <div className="flex gap-2">
            <input
              ...
              maxLength={500}
            />
            <button type="submit" className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg flex items-center gap-2 transition-colors">
              <Send size={20} />
            </button>
          </div>
          {canModerate && <p className="text-xs text-purple-400/60 mt-2">You can delete any message as a {rank}.</p>}
        </form>
      </main>
    </div>
  );
